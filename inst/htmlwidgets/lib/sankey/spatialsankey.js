...
d3.spatialsankey = function() {
  ...
  spatialsankey.ranges = function() {
    ...
    nodes = nodes.map(function(node) {
      ...
      var inflows = links.filter(function(link) { return link.target == node.properties.CODE; });
      var outflows = links.filter(function(link) { return link.source == node.properties.CODE; });
  ...
  spatialsankey.links = function(_) {
    ...
    links = links.map(function(link){
      ...
      var source_feature = nodes.filter(function(node) { return node.properties.CODE == link.source; })[0],
          target_feature = nodes.filter(function(node) { return node.properties.CODE == link.target; })[0];
     ...
     // console.log(flows);
      ...
      var flow = flows.filter(function(flow) { return flow.properties.CODE == link.properties.CODE; })[0];
      if (flow) {
        link.flow = flow.flow;
        //link.flows = flows.flow;
      }
